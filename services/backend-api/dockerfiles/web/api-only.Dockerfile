...
COPY package*.json ./
RUN npm install -g npm@12.0.1 && npm config set foreground-scripts true && npm install --legacy-peer-deps --workspaces=false --include-workspace-root=false --foreground-scripts
